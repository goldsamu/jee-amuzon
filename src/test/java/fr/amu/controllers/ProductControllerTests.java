...
public class ProductControllerTests
{
	...
	public void add() throws Exception
	{
		mvc.perform(post("/add").contentType(MediaType.APPLICATION_FORM_URLENCODED_VALUE).param("title", "value")
				.param("category", "value").param("description", "value")).andExpect(view().name("redirect:/"));
	}

	@Test
	public void remove() throws Exception
	{
		mvc.perform(post("/remove").contentType(MediaType.APPLICATION_FORM_URLENCODED_VALUE).param("productId", "0"))
	...
	{
		mvc.perform(post("/category").contentType(MediaType.APPLICATION_FORM_URLENCODED_VALUE).param("category", "value"))
